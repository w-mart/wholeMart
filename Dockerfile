...
RUN apt-get update && apt-get install -y maven curl socat
WORKDIR /app
COPY . .
...
# Healthcheck will probe the local actuator health endpoint via the external PORT (proxied to the app).
